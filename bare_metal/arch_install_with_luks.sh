...
#part1
printf '\033c'
echo "Welcome to iamnormi's arch installer script"
...
cryptsetup open $partition root
...
sed -i 's/^HOOKS=(.*/HOOKS=(base vdev autodetect microcode modconf kms keyboard keymap consolefont block encrypt filesystems fsck)/' /etc/mkinitcpio.conf
mkinitcpio -P
passwd
...
#find new name of "ttf-joypixels"
pacman -Sy --noconfirm xorg-server xorg-xinit xorg-xkill xorg-xsetroot xorg-xbacklight xorg-xprop \
     ...
     xorg-xrandr zsh-syntax-highlighting zsh-autosuggestions  xdg-user-dirs libconfig elinks vim ueberzug lvm2
...
cd $HOME
#dotfiles
git clone https://github.com/iamnormi/dotfiles --depth 1  ~/.local/src/dotfiles
